...
async def get_db():
    ...
    # Import garante o registro do listener `after_begin` (rls_context) que aplica
    # `SET LOCAL app.org_id` em CADA transação a partir do contexto do request —
    # cobre múltiplas transações por request. No-op sem org no contexto.
    import api.db.rls_context  # noqa: F401 — efeito colateral: registra o listener
